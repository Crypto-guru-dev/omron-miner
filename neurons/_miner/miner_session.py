...
class MinerSession:
    ...
    def _get_cached_circuit(model_id: str):
        """Cache circuit lookups to avoid repeated store access."""
        return circuit_store.get_circuit(str(model_id))

    @staticmethod
    def _optimized_json_dumps(data: dict) -> str:
        """Optimized JSON serialization with pre-allocated buffer."""
    ...
    def queryZkProof(self, synapse: QueryZkProof) -> QueryZkProof:
        """
        Optimized proof generation endpoint with reduced overhead.
        """
        ...
        time_in = time.perf_counter()  # More precise timing
        bt.logging.debug("Received optimized request from validator")
        ...
        try:
            # Use cached circuit lookup
            circuit = self._get_cached_circuit(str(model_id))
            if not circuit:
                raise ValueError(
            ...
            bt.logging.info(f"Running optimized proof generation for {circuit}")
            
            # Optimized session creation
            model_session = VerifiedModelSession(
                GenericInput(RequestType.RWR, public_inputs), circuit
            )
            bt.logging.debug("Optimized model session created successfully")
            ...
            synapse.query_output = self._optimized_json_dumps(response_data)
            
            bt.logging.trace(f"Optimized proof: {synapse.query_output}, Time: {proof_time}")
            model_session.end()
            try:
                bt.logging.info(f"✅ Optimized proof completed for {circuit}\n")
            except UnicodeEncodeError:
                bt.logging.info(f"Optimized proof completed for {circuit}\n")
        except Exception as e:
            ...
            traceback.print_exc()
            proof_time = time.perf_counter() - time_in
        ...
        bt.logging.info(
            f"Optimized response - Total: {delta_t:.3f}s, Proof: {proof_time:.3f}s, "
    ...
    def handle_pow_request(
        ...
        """
        Optimized proof of weights request handler.
        """
        if cli_parser.config.competition_only:
            bt.logging.info("Competition only mode enabled. Skipping proof generation.")
            synapse.query_output = "Competition only mode enabled"
        ...
        bt.logging.debug("Received optimized proof of weights request from validator")
        ...
        try:
            # Use cached circuit lookup
            circuit = self._get_cached_circuit(str(synapse.verification_key_hash))
            if not circuit:
                raise ValueError(
            ...
            bt.logging.info(f"✅ Optimized proof of weights completed for {circuit}\n")
        except Exception as e:
            bt.logging.error(
        ...
        bt.logging.info(
            f"Optimized PoW response - Total: {delta_t:.3f}s, Proof: {proof_time:.3f}s, "
            f"Overhead: {overhead_time:.3f}s."
        )
